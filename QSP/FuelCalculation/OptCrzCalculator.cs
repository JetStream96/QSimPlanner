    ...
    public class OptCrzCalculator
	{
		...
		private void loadFromTxt(string s)
		{
			...
			for (int i = 1; i <= lines.Length - 1; i++) {
				x = lines[i].Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
				weight[i - 1] = Convert.ToDouble(x[0]) *( wtUnitIsKG ? 1.0 : AviationConstants.LB_KG);
		...
		{
			//FT
			return Interpolation.Interpolate(weight, wt, alt, ArrayOrder.Decreasing);
		}
